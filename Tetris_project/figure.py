...
class Figure:
    ...
    def collide_from_right(self):  # Проверка на наличие фигуры справа от заданной
        for x, y in self.chosen_points:
            if self.top + y >= 0 and self.left + x + 1 < self.board.width and self.board.field[self.top + y][self.left + x + 1] == 1:
                return True
    ...
    def collide_from_left(self):  # Проверка на наличие фигуры слева от заданной
        for x, y in self.chosen_points:
            if self.top + y >= 0 and self.left + x - 1 >= 0 and self.board.field[self.top + y][self.left + x - 1] == 1:
                return True
